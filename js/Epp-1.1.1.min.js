...
function paint_cirl(obj_default_class, obj_change_class, start_color, end_color, main_class) { //画圆
	...
	var te = setInterval(function() {
		ite++;
		if (ite >= 112 || ite >= 111 / $(main_class + " .sp1").text() * $(main_class + " .sp0").text()) {
			clearInterval(te);
		}
...
function hover_down_menu(oli_obj, showDiv) { //下拉菜单效果
	$(oli_obj).hover(function() {
		...
		for (var i = 0; i < olen; i++) {
			setTimeout(function() {
				$(oli_obj).eq(ind).find(showDiv + " a").eq(j - 10).css({
	...
	function Flip(e) {
		...
		for (var i = 0; i < olen; i++) {
			setTimeout(function() {
				$(oli_obj).eq(ind).find(showDiv + " a").eq(j - 10).css({
					"transform": "translate(0,-" + j + "px)  rotate(" + -(80 + j + 5 + j * 5) + "deg)",
					"opacity": "1"
				});
				j = j + 1;
			}, 50 * i + 50)
...
function roll_picture(oLi) { //图片列表向左滚动
	...
	$(oLi).parent().css({
		"position": "absolute",
		"left": "0",
		"width": oLi_len * (oLi_wid + oLi_marl) + "px"
	});
...
function suspension(click_obj, move_obj, distance, className, negative) { //悬浮  1.点击的对象  2.移动的对象  3.距离右边的数值 4.类名 5.负值
	$(click_obj).click(function() {
		...
		if ($(this).attr('opene') == '1') {
			$(move_obj).animate({
				'right': -$(move_obj).width() - negative + "px"
			}, 600, 'swing');
			$(this).attr('opene', '0');
...
function full_screen(number, click_left, click_right, click_cirl) { //全屏左右滚动轮播 1.数量  2.左点击  2.右点击  3.小圆点点击
	...
	$(click_right).click(function() {
		thinde1++;
		if (thinde1 < olen1) {
			$(number).parent().animate({
				"left": -thinde1 * bowidth1 + "px"
			}, 600, "swing");
		}
	})
	$(click_left).click(function() {
...
function autobox(obj, oble, value, pase) { //居中 1.对象  2. 0-[左边] 1-[右边]  3.宽度  4.偏移量
	...
	function lrauto() {
		...
		if (oble == 0) {
			$(obj).css({
				left: (bwidt - value) / 2 + pase + "px"
			});
		} else {
			if (oble == 1) {
				$(obj).css({
					right: (bwidt - value) / 2 + pase + "px"
				})
			} else {
...
function Cutimg(obj) { //裁图后 垂直水平居中  1.图片对象
	...
	function autoimg() {
		...
		$(obj).css({
			"left": (widt - img_wid) / 2 + "px",
			"top": (heit - img_hei) / 2 + "px",
			"position": "absolute"
		});
		$(obj).parent().css({
			"position": "relative",
...
function two_scroll(number, cli_left, cli_right, click_cirl, textp, ovalue, ots, ohei) { //滚动轮播 1.元素的个数	2.左点击对象	3.右点击对象	4.小圆点对击对象[可选项] 5.文字变化 6.滚动的距离[必选项] 7.滚动的个数[必选项] 8.最外层高度
	...
	$(cli_left).click(function() {
		...
		$(textp).animate({
			"top": -j * $(textp).parent().outerHeight() + "px"
		});
	})
...
function fade(banner_ul_li, cirl, timer, click_left, click_right, cg_text) { //淡入淡出轮播  1.切换的对像	2.小圆点控制的对像	 3.时间值[必选]  4.左点击  5.右点击 6.文字部分切换
	...
	$(banner_ul_li).eq(i).css("zIndex", "1").animate({
		"opacity": "1"
	}, 600, 'swing');
	$(cirl).click(function() {
		$(this).addClass('acti').siblings().removeClass('acti');
		...
		$(banner_ul_li).eq(i).css("zIndex", "1").animate({
			"opacity": "1"
		}, 600, 'swing').siblings().css("zIndex", "0").animate({
			"opacity": "0"
		}, 600, 'swing');
	})
...
function phone_menu(read_div, add_pos) { /*手机模式菜单结构*/
	var menuHtml = $(read_div).html();
	$(add_pos).append('<div class="min-menu"><h2><i></i></h2><ul>' + menuHtml + '</ul></div>');
	var opnte = true;
...
function seamless(obj, prev, next, title, delay) { //无缝滚动轮播
	...
	$(obj).each(function(index, element) {
		$(this).css({
			"left": index * ($(this).width() + mlval) + "px"
		})
	});
	var poi = 0;
	var rocn = 0;
...
function Independent(main_div, structure, downClass) {
	...
	$(main_div).hover(function() {}, function() {
		$(downClass).hide();
	});
}
